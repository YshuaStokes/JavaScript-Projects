...
function placeXOrO(squareNumber) {
    ...
    if (!selectedSquares.some(element => element.includes(squareNumber))) {
        ...
        if (activePlayer === 'X') {
            ...
            //Active player may only be X or O so, if not X it myst be O
        } else {
        ...
        //This condition checks to see if it is comuters turn.
        if (activePlayer === 'O') {
    ...
    function computersTurn() {
        ...
        while (!success) {
            //A random number evaluated returns true, the square hasn't been selected yet.
...
function drawWinLine(coordX1, coordY1, coordX2, coordY2) {
    ...
    function animateLineDrawing() {
        ...
        if (x1 <= x2 && y1 <= y2) {
            //This condition adds 10 to the previous end x points.
            if (x < x2) { x += 10; }
            //This condition adds 10 to the previous end y point.
            if (y < y2) { y += 10; }
            //This condition cancels our animation loop
            //if we've reach th end points.
...
function resetGame() {
    ...
    //This resers our array so it is empty and we can start over.
    selectedSquares = [];
}